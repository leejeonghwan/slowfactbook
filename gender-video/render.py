...
S=json.loads((P/'scenes.json').read_text()); F={x['slide']:x for x in json.loads((P.parent/'data/full.json').read_text())['items']}
FF='/opt/homebrew/bin/ffmpeg'; FP='/opt/homebrew/bin/ffprobe'; SCALE=1.5; W,H=1920,1080; FPS=30
BG='#10252c'; FG='#f3f6f4'; MUT='#a7bdc3'; TEAL='#48d9b0'; BLUE='#f2d36d'; GRID='#294048'; ORANGE='#ef987a'
...
def text(d,x,y,s,size=24,color=FG,bold=False,anchor=None):d.text((xy(x),xy(y)),str(s),font=font(size,bold),fill=color,anchor=anchor)
...
def captions(s):
 chunks=[]
 for sentence in re.split(r'(?<=[.!?])\s+',s['narration']):chunks.extend(wrap(sentence,43))
...
def frame(i,t):
 ...
 if id=='opening':
  ...
  text(d,85,323,f'{53.1*p:.1f}%',51,BG,True);text(d,1150,323,f'{46.9*p:.1f}%',51,FG,True,'ra')
  text(d,64,438,'근속 18.6%   산업 17.1%   기타 관측 특성 17.4%',23,TEAL)
 ...
 for a,b,c in caps[i]:
  if a<=t<b:
   tw=d.textbbox((0,0),c,font=font(23))[2]/SCALE;rect(d,(640-tw/2-20,582,640+tw/2+20,623),'#06191e',5);text(d,640,589,c,23,FG,False,'ma');break
...
if __name__=='__main__':
 ...
 (P/'narration.md').write_text('# 격차의 구조 — 내레이션\n\n'+ '\n\n'.join(f'## {s["chapter"]}\n\n{s["narration"]}\n\n출처: {s["source"]}' for s in S))
 thumbs=[]
 for i,s in enumerate(S):
  im=frame(i,min(s['duration']-.6,8));im.save(P/'frames'/f'{i:02}.png');thumbs.append(im.resize((640,360)))
 sheet=Image.new('RGB',(1920,1080),BG)
 for i,im in enumerate(thumbs):sheet.paste(im,(i%3*640,i//3*360))
 sheet.save(P/'storyboard.jpg',quality=92)
 if '--stills' in sys.argv:sys.exit()
 cmd=[FF,'-hide_banner','-loglevel','error','-y','-f','rawvideo','-vcodec','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(FPS),'-i','pipe:0','-i',str(P/'narration.wav'),'-c:v','libx264','-preset','veryfast','-crf','20','-pix_fmt','yuv420p','-c:a','aac','-b:a','192k','-movflags','+faststart','-shortest',str(P/'gender-gap-explained.mp4')]
