...
def frame(i,t):
 ...
 for a,b,c in caps[i]:
  if a<=t<b:
   rows=wrap(c,48)
   for j,row in enumerate(rows):text(d,640,578+j*27,row,21,FG,False,'ma',False)
   break
...
if __name__=='__main__':
 ...
 thumbs=[]
 for i,s in enumerate(S):
  im=frame(i,min(s['duration']-.6,8));im.save(P/'frames'/f'{i:02}-v2.png');thumbs.append(im.resize((640,360)))
 sheet=Image.new('RGB',(1920,1080),BG)
 for i,im in enumerate(thumbs):sheet.paste(im,(i%3*640,i//3*360))
 sheet.save(P/'storyboard-v2.jpg',quality=92)
 if '--stills' in sys.argv:sys.exit()
 cmd=[FF,'-hide_banner','-loglevel','error','-y','-f','rawvideo','-vcodec','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(FPS),'-i','pipe:0','-i',str(P/'narration.wav'),'-c:v','libx264','-preset','veryfast','-crf','20','-pix_fmt','yuv420p','-c:a','aac','-b:a','192k','-movflags','+faststart','-shortest',str(P/'gender-gap-explained-v2.mp4')]
